...
kable(destable)
```
Below is a graph that shows the type and missing values in the COVID 19 dataset. As we can see from the graph, there are only few missing values, and we would use none of variables that contain missing values in this report.
```{r}
